...
def test_stamp_preserves_entries_and_attests(tmp_path, monkeypatch) -> None:
    ...
    stamp_apk(apk=apk, build_recipe_path=build_recipe, registry_path=registry, dev_id="dev_1", build_id="build_1",
    ...
    assert read_trust_statement(output)["unsignedSha256"] == hashlib.sha256(apk.read_bytes()).hexdigest()
