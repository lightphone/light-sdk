...
from .keys import KEY_ALIAS, PASSWORD_ENV, STAMP_PASSWORD_ENV, keystore_path, require_password
...
def sign_apk(*, apk: Path, build_recipe_path: Path, registry_path: Path, build_id: str,
             keys_dir: Path, stamp_keystore: Path, stamp_key_alias: str, output: Path,
             metadata_output: Path, apksigner: Path) -> dict[str, object]:
    ...
    run_tool([
        ...
        "--key-pass", f"env:{PASSWORD_ENV}",
        "--stamp-signer", "--ks", str(stamp_keystore),
        "--ks-key-alias", stamp_key_alias, "--ks-pass", f"env:{STAMP_PASSWORD_ENV}",
        "--key-pass", f"env:{STAMP_PASSWORD_ENV}",
        "--out", str(output), str(apk),
    ])
